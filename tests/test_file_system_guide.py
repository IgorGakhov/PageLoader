...
@pytest.mark.parametrize('url, expected_file_path, expected_dir_path,\
    ...
    (
        ...
        'example-com.html', 'example-com_files',
        'example-com.ext', 'example-com_ext'
    ),
    (
        'https://example.com',
    ...
    (
        ...
        'example-com-path.html', 'example-com-path_files',
        'example-com-path.ext', 'example-com-path_ext'
    ),
    (
        'https://example.com/path1/path2',

        'example-com-path1-path2.html', 'example-com-path1-path2_files',
        'example-com-path1-path2.ext', 'example-com-path1-path2_ext'
    ),
    (
        'https://example.com/path1/path2.css',

        'example-com-path1-path2.css', 'example-com-path1-path2_files',
        'example-com-path1-path2.ext', 'example-com-path1-path2_ext'
    ),
    (
    ...
    (
        ...
        'example-com-80-path.php', 'example-com-80-path_files',
        'example-com-80-path.ext', 'example-com-80-path_ext'
    ),
])
def test_get_paths(
    url, expected_file_path, expected_dir_path,
    expected_file_path_with_custom_ext, expected_dir_path_with_custom_ext,
    tmp_path: pathlib.Path
):
    assert get_file_path(url, tmp_path) == \
        os.path.join(tmp_path, expected_file_path)
    assert get_file_path(url, tmp_path, ext='ext') == \
        os.path.join(tmp_path, expected_file_path_with_custom_ext)

    assert get_dir_path(url, tmp_path) == \
        os.path.join(tmp_path, expected_dir_path)
    assert get_dir_path(url, tmp_path, ext='ext') == \
        os.path.join(tmp_path, expected_dir_path_with_custom_ext)
